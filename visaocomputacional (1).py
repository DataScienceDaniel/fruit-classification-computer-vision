...
"""###### Criação da coluna target chamado Label que é a que queremos classificar"""
...
image_tomato_wb_rgb = cv2.cvtColor(image_tomato_wb, cv2.COLOR_BGR2RGB)

plt.imshow(image_tomato_wb_rgb)
...
def copy_images(sub_df, subset_name):
    for _, row in sub_df.iterrows():
        label_folder = f"dataset/{subset_name}/{row['Label']}"
...
samples = random.sample(pred_rows, min(12, len(pred_rows)))
cols = 4
...
for i, row in enumerate(samples, 1):
    ...
    plt.imshow(img)
    plt.title(f"Pred: {row['pred']} | True: {row['true']}")
    plt.axis("off")
plt.tight_layout()
plt.show()
...
print(f"Found {len(bagged_images)} bagged images.")

images = random.sample(bagged_images, min(6, len(bagged_images)))
...
plt.suptitle("YOLOv8 Classification — Fruits Inside Bags", fontsize=16, y=1.02)
plt.tight_layout()
plt.show()
